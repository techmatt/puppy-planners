		...
		public Map()
        {
			...
			for(int x = 0; x < data.GetUpperBound(0); x++)
                for(int y = 0; y < data.GetUpperBound(1); y++)
                {
                    ...
                    c.scoutCost = GameFunctions.scoutCost(c.coord.x + 0.5 - Constants.mapSize / 2.0,
			...
			data[d + 0, d + 0].building = new Building(Database.Get.buildings["field"]);
        ...
        public void scoutCell(Coord c, int scoutRadius, double scoutValue)
        {
            ...
            for(int xOffset = -scoutRadius; xOffset <= scoutRadius; xOffset++)
                for(int yOffset = -scoutRadius; yOffset <= scoutRadius; yOffset++)
                {
                    ...
                    if(!cell.explored)
                    {
						cell.scoutProgress += scoutValue / regionsToScout;
						if(cell.scoutCost <= cell.scoutProgress)
                        {
                            cell.explored = true;
							cell.scoutProgress = cell.scoutCost;
                        }
                    }
                }
        }
